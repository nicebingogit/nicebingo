...
| 🤖 **Smart bot fill** | The single room is filled with 18-140 invisible bots each round (the count is **re-rolled every round** from the number of **humans** playing: fewer humans → more bots). Bots join EVERY round — a room with **zero humans still plays a full round** with bots. Each bot carries a random 1-3 real cards (capped to the pool), feed the pool, and are invisible to players — **only the super admin knows they are bots**; everyone else just sees players with human-like names — **20% Oromo / 20% Amhara / 10% Tigray / 30% general Ethiopian male / 5% Ethiopian female / 10% East African / 5% international nicknames** (Guyo, Lemma, Merhawi, Girum, Hiwot, Baraka, HotShot…). The fill is **self-healing** at boot and throughout the preparation countdown, but the roster and prize pool are **frozen** once a round starts — never change mid-round |
