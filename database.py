...
class Database:
    ...
    # --------------------------------------------------------- house profit
    def get_total_approved_deposits(self) -> int:
        """Sum of all approved deposits (real money flowing INTO the system)."""
        with self._session() as conn:
            row = conn.execute(
    ...
    def get_total_prizes_paid(self) -> int:
        """Sum of all prizes paid to winners (real money flowing OUT)."""
        with self._session() as conn:
            row = conn.execute(
                "SELECT COALESCE(SUM(prize_paid), 0) FROM games "
                "WHERE status = 'finished'"
            ).fetchone()
            return int(row[0])

    def get_total_bets_collected(self) -> int:
        """Sum of all bets collected from real players across all rounds."""
        with self._session() as conn:
            row = conn.execute(
                "SELECT COALESCE(SUM(total_bet), 0) FROM game_history"
            ).fetchone()
            return int(row[0])

    def house_profit_summary(self) -> Dict:
        """Full house profit breakdown."""
        deposits = self.get_total_approved_deposits()
        prizes = self.get_total_prizes_paid()
        bets = self.get_total_bets_collected()
        # house profit = deposits + bot fake bets - prizes paid
        # We only count real deposits as revenue; bot bets inflate the pool
        # but never represent real money. The house keeps the difference.
        house_profit = deposits - prizes
        profit_pct = (house_profit / deposits * 100) if deposits > 0 else 0
        return {
            'total_deposits': deposits,
            'total_prizes_paid': prizes,
            'total_bets_collected': bets,
            'house_profit': house_profit,
            'house_profit_pct': round(profit_pct, 1),
        }
