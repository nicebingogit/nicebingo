...
* A single card is rendered **bigger for easy tapping** (the `small` variant,
  capped at `max-width: 320px`, centered, 14 px cells) so the card, daub
  hint and BINGO button all stay visible without scrolling.
* Playing **2–3 cards** uses the bigger `triple` variant (15 px cells) and 3
...
gradually during the preparation countdown** (one bot per tick, so players
see them joining one-by-one as if real humans are picking cards) and again at
`start_round` (`ensure_minimum_players`), up to
...
* **They join sequentially:** during the preparation countdown, the game loop
  adds **one bot per tick** (every second) instead of all at once, so the
  player sees the bot count grow gradually — mimicking real players joining
  the room.
...
The `_require_super_admin()` guard checks membership in `SUPER_ADMIN_IDS`
**directly** — the caller does NOT need to be in `ADMIN_IDS` or have
`is_admin=1` in the DB. This means a super admin can access the super admin
console immediately, even before being promoted to regular admin.
...
reason (≤ 500 chars) goes to **every Super Admin**, who are notified by bot
message and resolve it in the 👑 Super console:
...
appeals(id, user_id, transaction_id, reason, status, resolution,
        ...
        -- status: pending | approved | rejected; resolved by ANY super admin
...
online tracking). `_require_super_admin()` checks `admin_id` membership in
`config.SUPER_ADMIN_IDS` **directly** — it does NOT require the caller to be
in `ADMIN_IDS` or have `is_admin=1` in the DB.
...
* **Super-admin endpoints are guarded by `config.SUPER_ADMIN_IDS`** — a normal
...
15. **bots win rounds**: all bot names are human names; a force-started
...
17. **multi-super-admin**: two different IDs can both access `/api/superadmin/*`;
    promoting a user to admin persists after page refresh (two-step upsert);
    super admin panel accessible without being in `ADMIN_IDS`.
...
| `PYTHONANYWHERE.md` | step-by-step free 24/7 deployment guide (PythonAnywhere) |
...
3. **Reload the Web app**: Go to the **Web** tab and click **Reload**.
   This is essential — the WSGI process must be reloaded to pick up code changes.
4. Verify the bot is working by sending `/start` in Telegram.

**Common issue: bot stops after `git pull`**
If the bot stops responding after pulling code, the WSGI process needs to be
reloaded. Go to the **Web** tab → click **Reload**. Check the **Error log**
if the reload fails. The webhook is automatically re-registered on startup.
...
card. Full click-by-click guide: **`PYTHONANYWHERE.md`**. The same image deploys
...
* **Sequential bot joins**: during preparation, bots join one per tick (every
  second) instead of all at once, capped at `MAX_TOTAL_PLAYERS`. Players see
  the bot count grow gradually.
